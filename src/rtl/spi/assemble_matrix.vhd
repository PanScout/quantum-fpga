	...
	-- Assuming 'dimension', 'sfixed36', 'csfixed36', 'cmatrix' are defined here
	...
	entity assemble_matrix is
		 port (
			  ...
			  data_in  : in  std_logic_vector( (2 * sfixed36'length) - 1 downto 0); -- e.g., 72 bits: [RE(35 downto 0) | IM(35 downto 0)]
	...
	architecture rtl of assemble_matrix is
		 ...
		 signal internal_matrix : cmatrix := (others => (others => ZERO_csfixed36));
		 ...
		 constant F_LEN       : integer := sfixed36'length; -- e.g., 36
		 ...
		 begin
			  if rising_edge(clk) then
					if reset = '1' then
						 ...
						 internal_matrix <= (others => (others => ZERO_csfixed36));
					else
						 ...
						 if current_state /= DONE and valid_in_rising_edge = '1' then  -- *** NEW Condition ***
							  ...
							  -- Ensure the conversion uses the correct bounds from sfixed36 type
							  ...
							  internal_matrix(row_index_cnt)(col_index_cnt) <= temp_csfixed36;
