...
architecture rtl of disassemble_psi_matrix is
    ...
    type state_type is (IDLE, SENDING_WAIT, SENDING_ADVANCE, DONE);
    signal current_state, next_state : state_type := IDLE;

    -- Constant for sfixed36 slicing length (e.g., 36 bits for sfixed36)
    constant F_LEN : integer := sfixed36'length;
...
begin
    ...
    -- Iterates over each psi_matrix element (each element is a csfixed36 within a cvector).
    ------------------------------------------------------------------------------
    process(clk)
    begin
         if rising_edge(clk) then
