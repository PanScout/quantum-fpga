...
architecture Behavioral of pade_numerator is
    ...
    -- Horner's method coefficients for -(((B+12)B+60)B+120)
		...
		im => to_sfixed(  0.0, sfixed36'High, sfixed36'Low)
		);
    ...
    begin
        for i in matrix'range loop
            for j in matrix(i)'range loop
                matrix(i)(j) := (
                    re => to_sfixed(0, sfixed36'high, sfixed36'low),
                    im => to_sfixed(0, sfixed36'high, sfixed36'low)
                );
            end loop;
        end loop;
        return matrix;
    ...
    function negate_cmatrix(matrix : cmatrix) return cmatrix is
        variable res : cmatrix;
    begin
        for i in matrix'range loop
            for j in matrix(i)'range loop
                res(i)(j).re := resize(-matrix(i)(j).re, sfixed36'high, sfixed36'low);
                res(i)(j).im := resize(-matrix(i)(j).im, sfixed36'high, sfixed36'low);
            end loop;
        end loop;
        return res;
    end function;
...
begin
    ...
    P <= negate_cmatrix(current_result);
